...
function Help.create_controls(window, ui)
    ...
    _create_link_box(window, ui, "help_github", GITHUB_LABEL_TEXT, GITHUB_URL, GITHUB_HEIGHT)
    _create_help_text(window, ui, "help_commands", commands_text, COMMANDS_HEIGHT)
end

function Help.register(window, ui)
    return {
        ui.add_title(TR("About LUI")),
        window.controls.help_about,
        ui.add_break(),
        window.controls.help_github,

        ui.add_hr(),
        ui.add_title(TR("Commands")),
        window.controls.help_commands,
    }
end
