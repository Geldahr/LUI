...
local function _add_font_identity_controls(page, base_key, font_label, size_label, style_label)
    ...
    page:add_line_edit(base_key .. "_font_size", size_label or TR["Font Size"])
...
local function _add_font_color_controls(page, base_key, color_label, outline_label)
    page:add_color_picker(base_key .. "_font_color", color_label or TR["Font Color"])
    page:add_color_picker(base_key .. "_font_outline_color", outline_label or TR["Outline Color"])
end
...
local function _add_vitals_label_controls(page, prefix, bar_key, label_index)
    ...
    page:add_row_break()
    page:add_line_edit(key .. "_text", TR["Text"], page.vital_format_help, true)
    page:add_row_break()
    ...
    page:add_row_break()
    page:add_line_edit(key .. "_x_offset", TR["X offset"])
    page:add_line_edit(key .. "_y_offset", TR["Y offset"])
    page:add_row_break()
...
local function _build_standard_frame_colors_form(page, prefix)
    page:add_color_picker(prefix .. "_morale_background_color", TR["Background Color"])
    page:add_color_picker(prefix .. "_border_color", TR["Border Color"])
end

local function _build_standard_morale_form(page, prefix)
    page:add_line_edit(prefix .. "_morale_height", TR["Bar Height"])
    page:add_row_break()
    page:add_line_edit(prefix .. "_morale_bubble_text", TR["Bubble Format (%B)"], page.bubble_format_help, true)
end

local function _build_standard_morale_colors_form(page, prefix)
    page:add_color_picker(prefix .. "_morale_bubble_color", TR["Bubble Color"])
    page:add_color_picker(prefix .. "_morale_color_neutral", TR["Neutral Color"])
    page:add_row_break()
    page:add_title(TR["Step Colors"])
    page:add_color_picker(prefix .. "_morale_color_high", TR["High Color"])
    page:add_color_picker(prefix .. "_morale_color_medium", TR["Medium Color"])
    page:add_color_picker(prefix .. "_morale_color_low", TR["Low Color"])
    page:add_row_break()
    page:add_color_picker(prefix .. "_morale_color_critical", TR["Critical Color"])
    page:add_break()
    page:add_title(TR["Gradient Colors"])
    page:add_checkbox(prefix .. "_morale_gradient", TR["Enable gradient colors"], true)
    page:add_row_break()
    page:add_color_picker(prefix .. "_morale_gradient_full", TR["Full Color"])
    page:add_color_picker(prefix .. "_morale_gradient_mid", TR["Mid Color"])
    page:add_color_picker(prefix .. "_morale_gradient_low", TR["Low Color"])
    page:add_row_break()
...
local function _build_standard_power_form(page, prefix, include_boss_fields)
    if include_boss_fields == true then
        ...
        page:add_row_break()
        page:add_line_edit(prefix .. "_power_width", TR["Width"])
        page:add_line_edit(prefix .. "_power_height", TR["Bar Height"])
        page:add_row_break()
...
local function _build_standard_power_colors_form(page, prefix)
    page:add_color_picker(prefix .. "_power_color", TR["Power Color"])
    page:add_color_picker(prefix .. "_wrath_color", TR["Wrath Color"])
end

local function _build_buffs_form(page, prefix)
    page:add_line_edit(prefix .. "_buff_size", TR["Icon Size"])
    page:add_row_break()
...
local function _build_debuffs_form(page, prefix)
    ...
    page:add_row_break()
    page:add_line_edit(prefix .. "_debuff_size", TR["Icon Size"])
    page:add_row_break()
...
local function _new_targets_target_colors_section(window, refresh_preview_fn)
    local frame = ConfigContent(window, 3, refresh_preview_fn)
    frame:add_color_picker("target_targets_target_background_color", TR["Background Color"])
    frame:add_color_picker("target_targets_target_border_color", TR["Border Color"])

    local morale = ConfigContent(window, 3, refresh_preview_fn)
    morale:add_color_picker("target_targets_target_bubble_color", TR["Bubble Color"])
    morale:add_color_picker("target_targets_target_color_neutral", TR["Neutral Color"])
    morale:add_row_break()
    morale:add_break()
    morale:add_title(TR["Step Colors"])
    morale:add_color_picker("target_targets_target_color_high", TR["High Color"])
    morale:add_color_picker("target_targets_target_color_medium", TR["Medium Color"])
    morale:add_color_picker("target_targets_target_color_low", TR["Low Color"])
    morale:add_row_break()
    morale:add_color_picker("target_targets_target_color_critical", TR["Critical Color"])
    morale:add_break()
    ...
    morale:add_row_break()
    morale:add_color_picker("target_targets_target_color_gradient_full", TR["Full Color"])
    morale:add_color_picker("target_targets_target_color_gradient_mid", TR["Mid Color"])
    morale:add_color_picker("target_targets_target_color_gradient_low", TR["Low Color"])
    morale:add_row_break()
...
local function _new_self_unit_page(window)
    ...
    frame:add_line_edit("self_width", TR["Frame Width"])
    frame:add_line_edit("self_border_width", TR["Border Width"])
    frame:add_row_break()
    frame:add_line_edit("self_effects_height", TR["Effects Height"])
    frame:add_row_break()
    ...
    frame:add_row_break()
    frame:add_line_edit("self_incombat_opacity", TR["In-combat opacity"])
    frame:add_line_edit("self_outcombat_opacity", TR["Out-of-combat opacity"])
    frame:add_row_break()
    frame:add_checkbox("self_ressource_background_matches_missing", TR["Matching background"], true)
    frame:add_row_break()
    frame:add_line_edit("self_ressource_background_dimming", TR["Dimming"])
    page:add_tab(TR["Frame"], "frame", frame)
...
local function _new_target_unit_page(window)
    ...
    local frame = ConfigContent(window, 4, page.refresh_preview)
    frame:add_line_edit("target_width", TR["Frame Width"])
    frame:add_line_edit("target_border_width", TR["Border Width"])
    frame:add_row_break()
    frame:add_line_edit("target_effects_height", TR["Effects Height"])
    frame:add_row_break()
    frame:add_dropdown("target_effects_position", TR["Effects Position"], frame.vitals_effects_position_labels,
        frame.vitals_effects_position_values)
    frame:add_row_break()
    frame:add_line_edit("target_incombat_opacity", TR["In-combat opacity"])
    frame:add_line_edit("target_outcombat_opacity", TR["Out-of-combat opacity"])
    frame:add_row_break()
    frame:add_checkbox("target_ressource_background_matches_missing", TR["Matching background"], true)
    frame:add_row_break()
    frame:add_line_edit("target_ressource_background_dimming", TR["Dimming"])
    page:add_tab(TR["Frame"], "frame", frame)
...
local function _new_boss_unit_page(window)
    ...
    local frame = ConfigContent(window, 4, page.refresh_preview)
    frame:add_line_edit("target_boss_width", TR["Frame Width"])
    frame:add_line_edit("target_boss_border_width", TR["Border Width"])
    frame:add_row_break()
    frame:add_line_edit("target_boss_effects_height", TR["Effects Height"])
    frame:add_row_break()
    frame:add_line_edit("target_boss_incombat_opacity", TR["In-combat opacity"])
    frame:add_line_edit("target_boss_outcombat_opacity", TR["Out-of-combat opacity"])
    frame:add_row_break()
    frame:add_checkbox("target_boss_ressource_background_matches_missing", TR["Matching background"],
        true)
    frame:add_row_break()
    frame:add_line_edit("target_boss_ressource_background_dimming", TR["Dimming"])
    page:add_tab(TR["Frame"], "frame", frame)
...
local function _new_targets_target_unit_page(window)
    ...
    local frame = ConfigContent(window, 4, page.refresh_preview)
    frame:add_line_edit("target_targets_target_width", TR["Frame Width"])
    frame:add_line_edit("target_targets_target_height", TR["Bar Height"])
    frame:add_line_edit("target_targets_target_border_width", TR["Border Width"])
    frame:add_row_break()
    frame:add_checkbox("target_targets_target_background_matches_missing", TR["Matching background"],
        true)
    frame:add_row_break()
    frame:add_line_edit("target_targets_target_background_dimming", TR["Dimming"])
    frame:add_row_break()
    frame:add_line_edit("target_targets_target_bubble_text", TR["Bubble Format (%B)"], frame.bubble_format_help,
        true)
    page:add_tab(TR["Frame"], "frame", frame)
...
local function _new_party_unit_page(window)
    ...
    frame:add_line_edit("party_width", TR["Frame Width"])
    frame:add_line_edit("party_border_width", TR["Border Width"])
    frame:add_row_break()
    frame:add_line_edit("party_rows", TR["Rows per Column"])
    frame:add_row_break()
    frame:add_line_edit("party_spacing_x", TR["Column Spacing"])
    frame:add_line_edit("party_spacing_y", TR["Row Spacing"])
    frame:add_row_break()
    frame:add_line_edit("party_incombat_opacity", TR["In-combat opacity"])
    frame:add_line_edit("party_outcombat_opacity", TR["Out-of-combat opacity"])
    frame:add_row_break()
    frame:add_checkbox("party_ressource_background_matches_missing", TR["Matching background"], true)
    frame:add_row_break()
    frame:add_line_edit("party_ressource_background_dimming", TR["Dimming"])
    page:add_tab(TR["Frame"], "frame", frame)
    ...
    icons:add_row_break()
    icons:add_line_edit("party_class_icon_size", TR["Icon Size"])
    icons:add_row_break()
    icons:add_line_edit("party_class_icon_x", TR["Icon X"])
    icons:add_line_edit("party_class_icon_y", TR["Icon Y"])
    icons:add_row_break()
    icons:add_checkbox("party_leader_icon_enabled", TR["Show leader icon"], true)
    icons:add_row_break()
    icons:add_line_edit("party_leader_icon_size", TR["Leader Icon Size"])
    icons:add_row_break()
    icons:add_line_edit("party_leader_icon_x", TR["Leader Icon X"])
    icons:add_line_edit("party_leader_icon_y", TR["Leader Icon Y"])
...
function VitalsPage:load()
    self:_set_all_loading(true)
    SelfVitals.load(self, self._settings, self.window._ui)
    TargetVitals.load(self, self._settings, self.window._ui)
    TargetBossVitals.load(self, self._settings, self.window._ui)
    TargetTargetsTarget.load(self, self._settings, self.window._ui)
    PartyLayout.load(self, self._settings, self.window._ui)
    PartyVitals.load(self, self._settings, self.window._ui)
    self:_set_all_loading(false)
    self:layout()
end

function VitalsPage:save()
    SelfVitals.apply(self, self._settings, self.window._ui)
    TargetVitals.apply(self, self._settings, self.window._ui)
    TargetBossVitals.apply(self, self._settings, self.window._ui)
    TargetTargetsTarget.apply(self, self._settings, self.window._ui)
    PartyLayout.apply(self, self._settings, self.window._ui)
    PartyVitals.apply(self, self._settings, self.window._ui)
end
