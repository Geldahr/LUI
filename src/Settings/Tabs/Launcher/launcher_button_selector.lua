...
local function _layout(entry)
    ...
    entry.available_fill:SetSize(available_w - (box_pad * 2), list_h)
    entry.available_list:SetPosition(0, 0)
    entry.available_list:SetSize(entry.available_fill:GetWidth() - SCROLL_W, list_h)
    ...
    entry.selected_fill:SetSize(selected_w - (box_pad * 2), list_h)
    entry.selected_list:SetPosition(0, 0)
    entry.selected_list:SetSize(entry.selected_fill:GetWidth() - SCROLL_W, list_h)
    ...
    local action_y = box_top + math.floor((box_h - action_block_h) / 2)
    ...
    local reorder_y = box_top + math.floor((box_h - reorder_block_h) / 2)
