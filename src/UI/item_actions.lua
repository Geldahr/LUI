...
--
-- Shared right-click actions menu for item names: every surface that shows
-- an item (Encyclopedia rows, bestiary card drops, ...) opens this same
-- menu and gets the same cross-window links. Entry presence is decided
-- here, at menu-open time, with O(1) store probes -- never during row
-- binds, so adding entries can never slow a list render.
