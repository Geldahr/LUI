import "LUI.src.UI.Settings.Tabs.Self.self_vitals_page"
...
local function _page(window)
    return window ~= nil and window._tab_pages ~= nil and window._tab_pages.self_vitals or nil
end
...
function SelfVitals.load(window, s, ui)
    ...
    controls.self_morale_font_name:set_value(v.morale.font.name)
    controls.self_morale_font_size.tb:SetText(tostring(v.morale.font.size))
    controls.self_morale_font_color.tb:SetText(ui.color_to_hex(v.morale.font.color))
    controls.self_morale_font_style:set_value(v.morale.font.style)
    controls.self_morale_font_outline_color.tb:SetText(ui.color_to_hex(v.morale.font.outline_color))
    ...
    controls.self_morale_text.tb:SetText(tostring(v.morale.string_format))
    controls.self_morale_text_alignment:set_value(v.morale.text_alignment)
    controls.self_morale_bubble_text.tb:SetText(tostring(v.morale.bubble_format))
    controls.self_morale_text_margin.tb:SetText(tostring(v.morale.text_margin))

    controls.self_power_height.tb:SetText(tostring(v.power.height))
    controls.self_power_font_name:set_value(v.power.font.name)
    controls.self_power_font_size.tb:SetText(tostring(v.power.font.size))
    controls.self_power_font_color.tb:SetText(ui.color_to_hex(v.power.font.color))
    controls.self_power_font_style:set_value(v.power.font.style)
    controls.self_power_font_outline_color.tb:SetText(ui.color_to_hex(v.power.font.outline_color))
    ...
    controls.self_power_text.tb:SetText(tostring(v.power.string_format))
    controls.self_power_text_alignment:set_value(v.power.text_alignment)
    controls.self_power_text_margin.tb:SetText(tostring(v.power.text_margin))
...
function SelfVitals.apply(window, s, ui)
    ...
    local morale_font_name = controls.self_morale_font_name:get_value()
    if type(morale_font_name) == "number" then
        v.morale.font.name = morale_font_name
    end
    local morale_font_size = tonumber(controls.self_morale_font_size.tb:GetText())
    if morale_font_size ~= nil then
        v.morale.font.size = morale_font_size
    end
    _apply_color(ui, v.morale.font.color, controls.self_morale_font_color.tb:GetText())
    local morale_font_style = controls.self_morale_font_style:get_value()
    if type(morale_font_style) == "number" then
        v.morale.font.style = morale_font_style
    end
    _apply_color(ui, v.morale.font.outline_color, controls.self_morale_font_outline_color.tb:GetText())
    ...
    local self_morale_text = controls.self_morale_text.tb:GetText()
    if type(self_morale_text) == "string" then
        v.morale.string_format = self_morale_text
    end
    local self_morale_text_alignment = controls.self_morale_text_alignment:get_value()
    if type(self_morale_text_alignment) == "number" then
        v.morale.text_alignment = self_morale_text_alignment
    end
    local self_morale_text_margin = tonumber(controls.self_morale_text_margin.tb:GetText())
    if self_morale_text_margin ~= nil then
        v.morale.text_margin = self_morale_text_margin
    end
    ...
    local power_font_name = controls.self_power_font_name:get_value()
    if type(power_font_name) == "number" then
        v.power.font.name = power_font_name
    end
    local power_font_size = tonumber(controls.self_power_font_size.tb:GetText())
    if power_font_size ~= nil then
        v.power.font.size = power_font_size
    end
    _apply_color(ui, v.power.font.color, controls.self_power_font_color.tb:GetText())
    local power_font_style = controls.self_power_font_style:get_value()
    if type(power_font_style) == "number" then
        v.power.font.style = power_font_style
    end
    _apply_color(ui, v.power.font.outline_color, controls.self_power_font_outline_color.tb:GetText())
    ...
    local self_power_text = controls.self_power_text.tb:GetText()
    if type(self_power_text) == "string" then
        v.power.string_format = self_power_text
    end
    local self_power_text_alignment = controls.self_power_text_alignment:get_value()
    if type(self_power_text_alignment) == "number" then
        v.power.text_alignment = self_power_text_alignment
    end
    local self_power_text_margin = tonumber(controls.self_power_text_margin.tb:GetText())
    if self_power_text_margin ~= nil then
        v.power.text_margin = self_power_text_margin
    end
