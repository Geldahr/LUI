...
function RaidConfigWindow:Constructor()
    ...
    self.fill_button = UI.Widgets.LuiButton()
    self.fill_button:SetParent(self.content)
    self.fill_button:set_text(TR["Fill from roster"])
    self.fill_button.Click = function()
        RaidConfig.seed_from_snapshot(GroupSnapshot.read(self.lp))
...
function RaidConfigWindow:apply_settings()
    ...
    self.fill_button:set_scale(scale)
    self.fill_button:set_font(font)
    self.fill_button:SetPosition(0, buttons_y)
    self.fill_button:SetSize(button_w, button_h)
    self.clear_button:set_scale(scale)
    self.clear_button:set_font(font)
    self.clear_button:SetPosition(button_w + section_gap, buttons_y)
